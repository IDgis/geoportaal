...
INSERT INTO gb.user VALUES
	(15, 'Test', '$2a$10$Ib3sL3FhwdGL1iLlmrWWiecivkQYRMWyW.MfrmHRav9gpNU.FxJ7e', 2);

drop materialized view if exists gb.metadata_search;

ALTER TABLE gb.metadata ALTER COLUMN location TYPE VARCHAR(2000);
...
DELETE FROM gb.supplier WHERE id=15;
DELETE FROM gb.user WHERE id=15;
drop materialized view if exists gb.metadata_search;
ALTER TABLE gb.metadata ALTER COLUMN location TYPE VARCHAR(100);
create materialized view gb.metadata_search as
select 
	m.id metadata_id,
	to_tsvector('dutch', coalesce((string_agg(m.title, ' ')), '')) ||
